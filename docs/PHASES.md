...
- ✅ (Sep 6) Patient experience round: multi-service booking (pick 2+ services, combined estimate, stored as joined service string); real clinic photos on Home (hero + Why Choose Us, AI-generated); explicit "Pay at clinic — nothing to pay online" on confirm + success (research: standard dental practice = no prepay for regular bookings; optional deposits only for high-value treatments, Phase 10 via Razorpay); portal upgraded with profile card, next-checkup recall card, upcoming/past appointment split, reschedule info, Book Appointment CTA. Fixed en route: portal recall lookup used wrong entity key (recalls vs recall) in demo + server — now tenant-scoped; live-verified multi-select (₹800+₹4,500=₹5,300) and portal recall card (Priya Sharma, 10 Sept 2026).
- ✅ (Sep 6) Multi-tenant settings isolation SHIPPED: settings now per-clinic (CityDent/WhiteField have own clinicName/hero/tagline/services; tenant 1 falls back to global public site settings). GET /settings is ALWAYS the public tenant-1 website — staff sessions can never alter public site content; staff pages (Settings, WebsiteMgr) use GET /settings/mine for their own clinic. PUT /settings writes to the caller's tenant only (tenant 1 = global). Dashboard goals/monthly now tenant-scoped (tenantSettings(t)) in demo + server. Smoke-tested live: CityDent admin sees CityDent settings, PUT doesn't touch SmileCraft or public site; public home stays SmileCraft even with CityDent session. Seed data: fresh demo dates (2 appts today, next-day bookings), Sep Paid invoice so Revenue This Month is non-zero. - Partly open in hardening: password hashing + JWT, staff CRUD UI in admin
